...
export class Recognizer {
    ...
    move(point, context) {
        ...
        if (context.points.length) {
            ...
            context.points = context.points.filter(point => Date.now() - point.t < 500)
            v = Math.sqrt((point.clientX - context.points[0].x) ** 2) + (point.clientY - context.points[0].y) ** 2;
        }
        d = v / (Date.now() - context.points[0].t)

        if (d > 1.5) {
            context.isFlink = true;
            this.dispatcher.dispatch('panend', {
                ...
                clientY: point.clientY,
                isVertical: context.isVertical,
                isFlink: context.isFlink,
                velocity: v
            })
        }else{
            context.isFlink = false;
        }
        if (context.isPan) {
            this.dispatcher.dispatch('panend', {
                startX: context.startX,
